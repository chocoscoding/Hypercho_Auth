...
import { addUser, addUserType, addCreatorType, addCreator, checkCreator, checkName } from "../services/Add";
...
export const Signupuser = async (req: CustomRequest, res: Response) => {
  ...
  if (!req.User) {
    try {
      ...
      return res.status(200).json({ status: 200, message: "User created successfully", userdata });
    } catch (error) {
      console.log(error);
  ...
  if (req.User) {
    try {
      ...
      if (checkIfCreatorExist) {
        return res.status(404).json({ status: 404, message: "This user is already a creator" });
      } else {
